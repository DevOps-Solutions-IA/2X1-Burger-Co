  ...
  assertPaymentCombination(fulfillment: OrderTicketType, preference: SofiaPaymentPreference) {
    const valid =
      (fulfillment === OrderTicketType.DELIVERY &&
        (preference === SofiaPaymentPreference.ONLINE || preference === SofiaPaymentPreference.CASH_ON_DELIVERY)) ||
      (fulfillment === OrderTicketType.TAKEAWAY &&
        (preference === SofiaPaymentPreference.ONLINE || preference === SofiaPaymentPreference.PAY_AT_PICKUP));
