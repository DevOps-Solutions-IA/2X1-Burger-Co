...
export default function DeliveryLoginPage() {
  ...
  const { user, loading, loginDelivery } = useAuth();
  ...
  return (
    <div
      data-testid="delivery-login-shell"
    ...
    >
      <div className="relative mx-auto grid min-h-[calc(100vh-2.5rem)] max-w-6xl items-start lg:items-center gap-6 lg:gap-12 lg:grid-cols-[1fr_420px]">
        <section className="flex items-center justify-center px-4 pt-6 pb-2 lg:py-0" aria-labelledby="delivery-login-brand">
        ...
        <section className="flex items-center justify-center px-4">
          <Card className="relative w-full max-w-lg overflow-hidden border-brand-500/20 bg-black p-8 text-stone-50 lg:p-10">
            ...
            <h2 className="mt-3 text-[1.9rem] font-bold text-white">Domiciliarios</h2>
            <p className="mt-2 text-[13px] leading-6 text-stone-400">Ingresa con tu nombre y codigo de acceso para gestionar entregas.</p>

            <form className="mt-8 space-y-5" onSubmit={handleSubmit(async (values) => {
              try {
                setError(null);
                const user = await loginDelivery(values.name, values.accessCode);
              ...
              <div>
                ...
                <Input data-testid="delivery-login-name" {...register('name')} placeholder="Tu nombre de acceso"
              ...
              <div>
                ...
                <Input data-testid="delivery-login-code" {...register('accessCode')} type="password" placeholder="Ingresa tu codigo"
              ...
              {error ? (
                <p role="alert" data-testid="delivery-login-error" className="rounded-2xl border border-red-400/25 bg-red-950/45 px-4 py-3 text-sm text-red-300">{error}</p>
              ) : null}
              <Button data-testid="delivery-login-submit" type="submit" size="lg" className="w-full" disabled={isSubmitting}>
                {isSubmitting ? 'Entrando...' : 'Entrar como domiciliario'}
              </Button>
            </form>
