...
test('installed-runtime SBOM is deterministic and scoped to the image root', () => {
  ...
  const apk = path.join(root, 'apk-installed');
  writeFileSync(apk, 'P:musl\nV:1.2.5-r0\n\nP:ca-certificates\nV:20250619-r0\n');
  ...
  const env = { ...process.env, SBOM_APK_DATABASE: apk };
  const first = execFileSync('node', [generator, 'api', 'a'.repeat(40), root], { encoding: 'utf8', env });
  const second = execFileSync('node', [generator, 'api', 'a'.repeat(40), root], { encoding: 'utf8', env });
  ...
  assert.ok(sbom.components.some((entry) => entry.name === '@scope/example' && entry.purl === 'pkg:npm/%40scope/example@1.2.3'));
  assert.ok(sbom.components.some((entry) => entry.name === 'runtime-app' && entry.version === '4.5.6'));
  assert.ok(sbom.components.some((entry) => entry.name === 'musl' && entry.purl.startsWith('pkg:apk/alpine/musl@')));
});
