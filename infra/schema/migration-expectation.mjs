...
export function resolveMigrationExpectation(migrationsRoot = path.resolve('prisma/migrations')) {
  ...
  for (const migration of migrations) {
    ...
    digest.update('\0');
    digest.update(readFileSync(migrationFile));
    digest.update('\0');
  }
...
export function evaluateAppliedMigrations(expectation, appliedRows) {
  ...
  for (const row of appliedRows) {
    ...
    if (row.finished === true && row.rolledBack !== true) successful.add(row.migrationName);
  ...
  const unexpected = [...successful].filter((name) => !expectation.migrations.includes(name)).sort();
  return Object.freeze({
    compatible: missing.length === 0 && unexpected.length === 0 && failed.length === 0,
