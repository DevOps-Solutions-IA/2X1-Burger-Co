...
const screenshotRoot = path.join(
  process.cwd(),
  'infra/environments/staging/selfhosted-data/deployment-prep/screenshots/audit-8b-login-full-execution-hardline',
);

const criticalRequestPattern = /\/api\/|\/login|\/brand\/sidebar-logo\.png/;

async function loginAdminFromUi(page: import('@playwright/test').Page) {
...
test.describe('AUDIT-8B login brand panel full execution hardline', () => {
  test('login preserves auth flow and upgrades brand panel without legacy cards', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Control real para una operación que no se detiene.' })).toBeVisible();
    await expect(page.getByText('2X1 Burger Co.')).toBeVisible();
    await expect(page.getByText('Ventas, caja e inventario conectados')).toBeVisible();
    ...
    await page.screenshot({
      path: path.join(screenshotRoot, 'desktop', 'login-desktop-1440x900.png'),
      fullPage: true,
    });
    await page.screenshot({
      path: path.join(screenshotRoot, 'after', 'login-after-desktop.png'),
      fullPage: true,
    });
    await page.screenshot({
      path: path.join(screenshotRoot, 'after', 'legacy-cards-removed-proof.png'),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: path.join(screenshotRoot, 'error-state', 'login-error-state.png'),
      fullPage: true,
    });
  ...
  test('login is responsive on tablet and mobile without horizontal overflow', async ({ page }) => {
    await page.setViewportSize({ width: 1024, height: 768 });
    await page.goto('/login');
    await expect(page.getByTestId('login-brand-logo')).toBeVisible();
    await expect(page.getByTestId('login-email')).toBeVisible();
    await page.screenshot({
      path: path.join(screenshotRoot, 'tablet', 'login-tablet-1024x768.png'),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: path.join(screenshotRoot, 'mobile', 'login-mobile-390x844.png'),
      fullPage: true,
    });
  });
});
